...
description: |-
  ...
  TSIG keys, and access-control lists on a Technitium DNS Server instance.
---
...
zone lists — enabling repeatable, version-controlled DNS configuration.
...
* `skip_tls_verify` - (Optional, Boolean) If `true`, TLS certificate verification is
  disabled. May be set via `TECHNITIUM_SKIP_TLS_VERIFY`. Default: `false`.
